...
def loss_function(recon_x, x, mu, logvar, beta):
    ...
    return 2*MSE + beta * KLD, MSE, KLD


def random_search(dataset, input_dim, device, n_iter=20, epochs=1000, airfoil_min=0, airfoil_max=0):
    best_params = None
    best_loss = float('inf')
    ...
    for i in range(n_iter):
        ...
        for epoch in range(epochs):
            ...
            for train_batch_data in train_loader:
                ...
                loss, mse_loss, kld_loss = loss_function(recon, train_batch_data[:, :199], mu, logvar, beta)
                loss.backward()
                optimizer.step()
            ...
            if (epoch + 1) % 10 == 0:
                print(
                    ...
                    f'MSE Loss: {mse_loss.item():.4f}, KLD Loss: {kld_loss.item():.4f}, Beta: {beta:.4f}')

        cvae.eval()
        with torch.no_grad():
            test_data_recon, mu, logvar = cvae(test_data[:, :199], test_data[:, 199].view(-1, 1))
            test_loss, test_mse, test_kld = loss_function(test_data_recon, test_data[:, :199], mu, logvar, beta)
...
def main_train():
    ...
    # compute the camber of the airfoils
    airfoil_flipped = np.fliplr(airfoil)
    camber = np.max((airfoil + airfoil_flipped) / 2, axis=1)
    camber_tensor = torch.FloatTensor(camber).to(device)
    ...
    best_params, best_loss = random_search(dataset, input_dim=199, device=device, n_iter=1, epochs=1000,
                                           airfoil_min=airfoil_min, airfoil_max=airfoil_max)
